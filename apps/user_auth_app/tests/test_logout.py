"""
Logout endpoint tests for the user authentication API.

Verifies that:
- logout requires authentication (access token cookie)
- successful logout clears JWT cookies
- logout still succeeds even if the refresh token cookie is invalid
"""
...
class LogoutTests(APITestCase):
    """
    Tests for POST /logout/ (cookie-based JWT logout).
    """
    ...
    def test_logout_requires_auth_returns_401_without_access_cookie(self):
        response = cast(Response, self.client.post(self.logout_url, {}, format="json"))
